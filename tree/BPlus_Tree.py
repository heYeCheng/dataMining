...
'''python的排序模块bisect , 使用这个模块的函数前先确保操作的列表是已排序的
	一棵m阶的B+树和m阶的B树的异同点在于：
      1.有n棵子树的结点中含有n-1 个关键字； (此处颇有争议，B+树与B树n棵子树的结点中含有n个关键字。暂先提供两个参考链接：①wikipedia http://en.wikipedia.org/wiki/B%2B_tree#Overview；②http://hedengcheng.com/?p=525。而下面B+树的图尚未最终确定是否有问题，请读者注意)
...
class BPTree(object):
	...
	def insert(self, key):
		...
		if pkey is not None:
			print pkey,'oo', self._tree._keys
...
class BPTreeNode(object):
	def __init__(self, keys, pointers, capacity):
		''' New BPTreeNode with keys=[key0,key1,..., keyK], pointers=[pointer0, pointer1,...,pointerK+1]  pointer0=[key0, key1,keyK] '''
	...
	def insert(self, key):
		...
		print 'du', self._keys, key
		# 这里  self._pointers[] 实际上就是 BPTreeLeaf ，insert 就是调用 BPTreeLeaf 的方法
...
class BPTreeLeaf(object):
	...
	def insert(self, key):
		...
		if len(self._keys) > cap:
			# 这里的 new_leaf 之所以不是 python 的地址，是因为调用了 __str__ 这个方法，直接返回  key 值
