...
1、插入
    1）先产生一个 node
    2）调用 ChooseLeaf 找出合适的插入位置
    3）将 node 合并到原来的叶子中
    4）调用 AdjustTree ：
        a）如果节点的叶子数量超过 M，则进行分裂
        b）否则向上合并，扩大父节点的面积


2、分裂
    1）创建 2 个节点
    2）调用 PickSeeds，找出合并后增加面积最大的 2 个矩形作为种子，分别放入 2 个节点中 leaf1, leaf2
    3）进入循环，遍历剩下的矩形
        a）防止其中的一组节点数小于 m
        b）调用 PickNext，将新节点尝试融入原来两组节点 leaf1, leaf2 中。若增加的面积越大，就表示越不能和该组节点相融合
    4）将新的两组节点和父节点进行融合


3、删除
    1）调用 FindLeaf 方法, remove 该节点
    2）对该组节点进行压缩
        a）判断该组的子节点数是否小于 m，若小于则移除此节点。并重新添加被移除的所有子节点
    3）压缩根


4、查找区域
    1）调用 Search 方法
    2）一直递归到叶子节点，然后判断有没有交集，有交集就返回该子节点
